...
class PageSpeedSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PageSpeed
        fields = [
            'id', 'page_path', 'full_url', 'overall_score',
...
class PageSpeedViewSet(BaseViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 调用工具类测试页面速度
        test_result = test_page_speed(page_path)
    ...
    @extend_schema(
        summary="获取页面速度统计信息",
        description="获取页面总数、平均评分、优秀页面数、待优化页面数",
        responses={
            200: {
                "type": "object",
                "properties": {
    ...
    def statistics(self, request):
        ...
        total_count = PageSpeed.objects.count()
        avg_score = PageSpeed.objects.aggregate(avg=Avg('overall_score'))['avg'] or 0
        excellent_count = PageSpeed.objects.filter(overall_score__gte=90).count()
        needs_improvement_count = PageSpeed.objects.filter(overall_score__lt=70).count()
    ...
    @extend_schema(
        summary="测试新页面",
        description="传入页面路径，自动测试页面速度并保存结果",
        request={
            "application/json": {
                "type": "object",
                "properties": {
